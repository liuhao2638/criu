...
class PX4Sih:
    ...
    def start_single_sih_sitl(self, instance_count):
        # 设定px4进程的工作目录
        px4_working_dir = f"{self.px4_working_dir}/instance_{instance_count}"
    ...
    def start_sih_sitl(self):
        ...
        # 清理并准备工作目录
        os.system(f"rm -rf {config['paths']['px4_working_dir']} && mkdir -p {config['paths']['px4_working_dir']}")
        # 清理所有系统中的 px4 进程
    ...
    def start_sih_sitl1(self):
        ...
        # 使用 ProcessPoolExecutor 来并行执行进程
        instances = list(range(self.instance_count))
        with multiprocessing.Pool() as pool:
            pool.map(self.start_single_sih_sitl, instances)
